...
We can use `fct_lump()` to select the top `n = 5` factors:
...
Let's detect the number of cores on the machine:
...
We want to tune our hyperparameters creating a 5-fold cross validation (10 would be too much for our hardware). We stratify our folds, too.

```{r}
set.seed(42)

# this bit does not work on my machine
nh_folds <-
  vfold_cv(nh_train,
...
And let's define the range of values to tune `under_ratio`. It normally takes values in the following range:

```{r}
under_ratio()
```

So we can set up a grid like the following, using `grid_regular()` from the `tidymodels` package `dials` (more details [here](https://dials.tidymodels.org/reference/grid_regular.html)).
...
logistic_predictions <- logistic_fit %>%
  ...
  predict(nh_test) %>%
  # bind cols with predicted class
  # bind_cols(logistic_fit %>% predict(nh_test)) %>%
